...
txLege <- list(
  ...
  districtPos = list(11,13),
  KHStateNamePos = list(14,20),
  partyPos = list(22,24),
...
KH <- list(
  legCodePos = list(4,8),
  stateCodePos= list(9,10),
  districtPos = list(11,12),
  KHStateNamePos = list(13,20),
  partyPos = list(21,23),
  legislatorNamePos = list(26,36),
  rollCallStartPos = 37
)
...
readVotes <- function(file,
  ...
  voteData <- substring(data,fmt$rollCallStartPos, test)
  n <- length(voteData)
  m <- nchar(voteData)[1]
  ...
  icpsrLegis <- as.numeric(substring(data,fmt$legCodePos[1], fmt$legCodePos[2]))

  ## party affiliation
  party <- as.numeric(substring(data,fmt$partyPos[1],fmt$partyPos[2]))
  ...
  state <- as.numeric(substring(data,fmt$stateCodePos[1], fmt$stateCodePos[2]))  ## icpsr code
  KHstateName <- substring(data,fmt$KHStateNamePos[1], fmt$KHStateNamePos[2])
  ...
  lnames <- substring(data,fmt$legislatorNamePos[1], fmt$legislatorNamePos[2])
  for(i in 1:n){
